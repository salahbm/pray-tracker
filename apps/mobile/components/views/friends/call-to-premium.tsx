  ...
  return (
    <Accordion type="multiple">
      {features.map((feature, index) => (
        <AccordionItem key={`${feature}-${index}`} value={`${feature}-${index}`}>
  ...
  return (
    <React.Fragment>
      ...
      >
        ...
        <Button className="bg-primary rounded-full py-4 px-8 w-full mt-8" onPress={handleLink}>
        ...
        <View className="bg-popover rounded-2xl mt-6 w-full p-6">
          ...
          >
            <Text className="text-primary text-center font-normal text-sm">
              {showAllFeatures ? t('Friends.Pro.LessFeatures') : t('Friends.Pro.MoreFeatures')}
            </Text>
          </TouchableOpacity>
