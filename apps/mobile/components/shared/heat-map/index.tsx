...
const HeatMap: React.FC<HeatMapProps> = props => {
  ...
  const renderMonth = useCallback(
    (month: string, monthData: { scores: number[]; dates: string[] }) => {
      ...
      return (
        ...
        >
          <Text className="text-sm font-semibold text-foreground mb-2 capitalize">{month}</Text>
  ...
  return (
    <ScrollView horizontal showsHorizontalScrollIndicator={false} ref={scrollRef}>
      {Object.entries(yearData).map(([month, monthData]) => renderMonth(month, monthData))}
    </ScrollView>
  );
};
