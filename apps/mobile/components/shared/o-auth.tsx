...
const OAuth = ({ onSuccess }: { onSuccess: () => void }) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      onSuccess();
      fireToast.success(t(`Responses.MessageCodes.${MessageCodes.SIGN_IN_SUCCESSFULLY}`));
    })();
  ...
  return (
    <View>
      ...
      >
        <Image source={IMAGES.google} className="size-5 mx-2 max-w-5 max-h-5 object-contain" />
