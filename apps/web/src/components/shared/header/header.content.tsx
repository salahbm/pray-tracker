...
const Header: React.FC<{ locale: string }> = ({ locale }) => {
  ...
  // remove locale from pathname to check if it's in whiteHeader
  const path = pathname.replace(`/${locale}`, "")
  const isWhite = (isWhiteStore || whiteHeader.some((path) => path === path)) && path !== ""
                  ...
                  {menu.map((item: { id: string; name: string; key?: string }, idx: number) => {
                    return (
                      <RevealUp key={item.id} index={idx}>
                        ...
                        >
                          <li className="typo-title-1 group relative pb-0.5">
                            {item.name}
                ...
                <RevealUp key="request" index={0} className="hidden md:block">
                  <Link href="/request" onClick={toggleMenu}>
                    ...
                    >
                      ...
                      <p className="typo-title-1 text-primary-100 relative whitespace-nowrap transition-colors duration-500 group-hover:text-white">
                        {t("common.request")}
                      </p>
                    </button>
                  </Link>
                ...
                <div className="flex flex-col gap-2 md:flex-row md:items-center md:gap-7 lg:gap-10">
                  <RevealUp key="company-profile" index={0}>
                    <a
                      ...
                      target="_blank"
                      className="typo-body-1 flex cursor-pointer items-center gap-3"
                    >
                  ...
                  <RevealUp key="email" index={1}>
                    <a href={`mailto:${appConfig.app.email}`} className="flex-center typo-body-1 cursor-pointer gap-3">
